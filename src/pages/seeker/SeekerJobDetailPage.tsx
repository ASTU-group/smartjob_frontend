...
export const SeekerJobDetailsPage = () => {
  const { id } = useParams();
  const { user } = useAuth();
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="container mx-auto px-4 py-8 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <Link to="/dashboard/seeker/jobs" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-primary mb-8 transition-colors">
                ...
                <div className="space-y-6">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {similarJobs.map((j: any, i: number) => (
                        <Link to={`/dashboard/seeker/jobs/${j.id}`} key={i}>
